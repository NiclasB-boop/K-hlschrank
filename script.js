...
function animation() {
    ...
    image.src = "Test.gif?" + new Date().getTime();
    setTimeout(animationEnd, 36*120)
}

function animationEnd() {
    image.src = "Test_end.png"
    image.addEventListener("click",Sound)
}
function setAudioTime() {
    audio.currentTime = document.getElementById("Zeit").value;
    document.getElementById("zeitLabel").innerText = document.getElementById("Zeit").value
}
function Sound() {
    if (audio.paused){
        audio.play()
